...
if ! sudo grep -q "^Cmnd_Alias MPROOTCMD =" /etc/sudoers; then
    ...
    echo "Cmnd_Alias MPROOTCMD = /usr/bin/at -l, /usr/bin/crontab * -l, /usr/sbin/fdisk -l, /usr/sbin/iptables-save, /usr/bin/netstat, /usr/sbin/ss, /usr/sbin/dmidecode, /usr/sbin/vgdisplay, /usr/sbin/lvdisplay, /usr/sbin/auditctl -[vl], /usr/sbin/grubby --info ALL" | sudo tee -a /etc/sudoers
fi

if ! sudo grep -q "^Cmnd_Alias MPFILECMD =" /etc/sudoers; then
    echo "Cmnd_Alias MPFILECMD = /usr/bin/egrep, /usr/bin/cat, /usr/bin/find *, /usr/bin/ls, /usr/bin/getfacl, /usr/bin/test" |sudo tee -a /etc/sudoers
fi

if ! sudo grep -q "^Cmnd_Alias MPEXCPTNSCMD =" /etc/sudoers; then
    echo "Cmnd_Alias MPEXCPTNSCMD = !/usr/bin/find *-exec*, !/usr/bin/find *-fprint*, !/usr/bin/find *-ok*, !/usr/bin/find *-delete*,  !/usr/bin/find *-fls*,  !/usr/sbin/ss *--diag*, !/usr/sbin/ss *-D*, !/usr/sbin/dmidecode *--dump*" | sudo tee -a /etc/sudoers 
fi 
...
ARCHIV=$(pwd)
sudo tar -xvf $ARCHIV/RED\ OS/bin.tar -C $DIRECTORIA >> $DIRECTORIA/.soft
...
if [[ "$(stat -c '%a' "/home/$USER_NEW")" != "700" ]]; then
  ...
  sudo chmod -R 700 "/home/$USER_NEW/"
